...
contract EarthToken is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public constant _name = "Earth Token";
    string public constant _symbol = "EARTH";
    ...
    address public constant volcano =
    ...
    constructor()
        ERC20Detailed("Earth Token", "EARTH", uint8(DECIMALS))
        Ownable()
    {
    ...
    function takeFee(
        ...
        _gonBalances[volcano] = _gonBalances[volcano].add(
            gonAmount.mul(volcanoFee).div(feeDenominator)
        );
        ...
        _gonBalances[nftRewardPoolReceiver] = _gonBalances[
            nftRewardPoolReceiver
        ].add(gonAmount.mul(nftRewardPoolFee).div(feeDenominator));
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHToTreasuryESFAndNFTBUSD = address(this).balance.sub(
            balanceBefore
        );
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        require(
            amountToSwap > 0,
            "There is no EARTH token deposited in token contract"
        );
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
    ...
    function withdrawBNBToTreasury() external onlyOwner {
        uint256 balance = address(this).balance;
        payable(treasuryReceiver).transfer(balance);
    }
